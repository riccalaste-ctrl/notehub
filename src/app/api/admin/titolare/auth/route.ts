...
export async function POST(request: NextRequest) {
  ...
  response.headers.set('Set-Cookie', serialize(TITOLARE_COOKIE, token, { httpOnly: true, secure: process.env.NODE_ENV === 'production' || Boolean(process.env.APP_URL?.startsWith('https://')), sameSite: 'strict', maxAge: 1800, path: '/' }));
  return response;
}
